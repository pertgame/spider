...
class Taobao:
    ...
    def __init__(self):
        ...
        self.goodsHeaders = {
            ...
            'User-Agent' : 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:35.0) Gecko/20100101 Firefox/35.0',
            'Referer' : 'https://buyertrade.taobao.com/trade/itemlist/list_bought_items.htm?spm=a1z09.2.1997525045.2.qq1EAh',
            'Content-Type': 'application/x-www-form-urlencoded',
            'Connection' : 'Keep-Alive',
            'origin':'https://buyertrade.taobao.com'        
        }
        #用户名
        self.username = 'thunder54007'
    ...
    def getFirstGoodsPage(self,pageIndex):
        goodsURL = 'http://buyertrade.taobao.com/trade/itemlist/listBoughtItems.htm?action=itemlist/QueryAction&event_submit_do_query=1' + '&pageNum=' + str(pageIndex) + "&pageSize=15"
        pre = 1
        if pageIndex > 1:
            pre = pageIndex - 1
        data = {"buyerNick":"", "dateBegin":"","dateEnd":"","lastStartRow":"","logisticsService":"", "options":0, "orderStatus":"","queryBizType":"","rateStatus":"","refund":"","sellerNick":"", "queryOrder":"desc","pageNum":pageIndex, "pageSize":15, "prePageNo":pre}    
        #goodsURL = 'https://buyertrade.taobao.com/trade/itemlist/asyncBought.htm?action=itemlist/BoughtQueryAction&event_submit_do_query=1&_input_charset=utf8'
        #goodsURL = 'http://buyertrade.taobao.com/trade/itemlist/listBoughtItems.htm?action=itemlist/QueryAction&event_submit_do_query=1' 
        req = urllib2.Request(goodsURL)
        response = self.newOpener.open(req)
        page =  response.read().decode('gbk')
        print "f url:",goodsURL
        print "f page " + str(pageIndex) + " len:" + str(len(page))
        return page
    #获得已买到的宝贝页面
    def getGoodsPage(self,pageIndex):
        ...
        goodsURL = 'https://buyertrade.taobao.com/trade/itemlist/asyncBought.htm?action=itemlist/BoughtQueryAction&event_submit_do_query=1&_input_charset=utf8'
        #goodsURL = 'http://buyertrade.taobao.com/trade/itemlist/listBoughtItems.htm?action=itemlist/QueryAction&event_submit_do_query=1' 
        req = urllib2.Request(goodsURL, urllib.urlencode(data), self.goodsHeaders)
        response = self.newOpener.open(req)
        page =  response.read().decode('gbk')
        print "url:",goodsURL
        print "page " + str(pageIndex) + " len:" + str(len(page))
        return page

    #获取所有已买到的宝贝信息
    def getAllGoods(self,pageNum):
        print "page:" + str(pageNum) + u"获取到的商品列表如下"
        for x in range(1,int(pageNum)+1):
            page = self.getGoodsPage(x)
            self.tool.getGoodsInfo3(page)
